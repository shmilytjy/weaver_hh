...
//	          htmlLayout_lowerCase = htmlLayout.toLowerCase();
      ...
      //分页并且又明细表
      if(isSplitPrint && htmlLayout.indexOf(ParseExcelLayout.BEGMARK)>-1){
          int size =otherPara_hs.get("size") == null ? 0 : ((Integer) otherPara_hs.get("size"));
          if(size ==0){
        	  size= getpageSize(htmlLayout);//获取分页页数 
          }
          if(pageNum==1 && size > 1){//页数大于2也 第一页隐藏明细表下方的字段
        	  htmlLayout=hiddenMark(htmlLayout);//
          }
      }
   ...
   * @return
   */
  public int getpageSize(String html){
	  int size=0;
	  RecordSet rs = new RecordSet();
	  String maintable = "";
	  ...
	  int requestid = Util.getIntValue(request.getParameter("requestid"), 0);
		...
		if(billDetailTable.size()<=0){
			size = 1;
			return size;
		}
		org.jsoup.nodes.Document doc = Jsoup.parse(html.toString(), "UTF-8");
		org.jsoup.nodes.Element oTable = doc.getElementsByClass("excelDetailTable").first();
		int groupid = Util.getIntValue(oTable.id().replace("oTable", ""), 0);
		String groupName = billDetailTable.get(groupid);
		rs.executeSql("select b.* from " + maintable+ " a," + groupName+ " b where a.id=b.mainid and a.requestid ="+ requestid + " order by b.id");
		int totalCount = rs.getCounts(); // 总记录数
   ...
   * @return
   */
  public String hiddenMark(String html){
	  String htmlLayout = html;
	  String resultHtml = "";
	  int length = htmlLayout.length();
	  int index = htmlLayout.lastIndexOf("</seniordetailmark>");
	  if(index >=0){
		  resultHtml = htmlLayout.substring(0,index)+htmlLayout.substring(index,length).replace("<tr", "<tr style=\"display:none;\" ");
	  }else{
		  resultHtml = htmlLayout;
	  }
	  return resultHtml;
  }
  /**
...
//		        int pos = htmlLayout_lowerCase.indexOf("$node" + nodeid_tmp + "$");
		        ...
		        while (pos > -1) {
		          ...
		          nodeRemar_tmp = getNodeMarkPrint(String.valueOf(nodeid_tmp), String.valueOf(requestid));
...
//		          pos = htmlLayout_lowerCase.indexOf("$node" + nodeid_tmp + "$");
 ...
 public String getNodeMarkPrint(String nodeid,String rqid){
	 RecordSet rs = new RecordSet();
	 String remark="";
	 String sql = "select b.lastname,operatedate from workflow_requestlog a,HrmResource b where a.operator=b.id and requestid='"+rqid+"' and nodeid='"+nodeid+"' and logtype <>'1'";
	 rs.executeSql(sql);
	 if(rs.next()){
		 remark=Util.null2String(rs.getString("lastname"));
		 remark=remark+"<br/>"+Util.null2String(rs.getString("operatedate"));
	 }
	 return remark;
 }
  /**
